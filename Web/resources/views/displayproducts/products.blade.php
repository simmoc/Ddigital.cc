...
@endif	  
	<div class="col-md-4 col-sm-12 col-xs-12">
		...
		<div class="product {{ $extra_class }}">
			...
			<div class="product-content">
				...
				<a href="{{ URL_DISPLAY_PRODUCTS_DETAILS.$product->slug.'/'.$product->id }}" class="product-title" title="{{$product->name}}"> {{getPhrase(strlen($product->name) < 20) ? getPhrase($product->name) : getPhrase(substr($product->name,0,15)).'...' }} </a>
	...
	@endforeach
	 <!-- <div class="products-button pull-right">						
						<a class="btn btn-primary" href="{{ URL_VIEW_MORE_PRODUCTS }}">{{ getPhrase('view_more', 'upper') }}</a>
					</div> -->
	@if( ! isset($nopagelinks) )
		{{ $products->appends(request()->input())->links('pagination/default') }}
	@endif
	@else
		<div class="col-md-4 col-sm-12 col-xs-12"> {{ getPhrase('no_products_found') }}
		</div>
	@endif
