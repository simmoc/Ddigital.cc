...
<nav class="navbar navbar-default navbar-fixed-top">
	<div class="container">
		<div class="navbar-header">
			...
			?>
           <img src="{{IMAGE_PATH_SITE_LOGO.getSetting('site_logo', 'site_settings')}}" alt="" height="46" width="auto"></a>
		...
		<div class="collapse navbar-collapse" id="myNavbar1">
			...
			@if( $menu->count() > 0)
				<ul class="nav navbar-nav navbar-right">
					@foreach( $menu->get() as $menu_item )
						@if( $menu_item->url == 'login' || $menu_item->url == 'register' )
							@if (Auth::check())
							<?php
								$user = getUserRecord();
							...
							@if( $menu_item->url == 'login' )
								<li {{ isActive($main_active, 'login') }}><a href="{{ $url }}">{{ getPhrase('my_dashboard') }}</a></li>							
								<li>	
								<a href="{{URL_MESSAGES}}">
							...
							@else
								@if( $menu_item->url == 'login' )	
								<li {{ isActive($main_active, 'login') }}><a href="{{ URL_USERS_LOGIN }}">Login</a></li>
								
								@endif
								
							@endif	
						@elseif( $menu_item->url == 'products-display' )
							<?php
							$categories = App\Category::where('status', '=', 'Active');
							?>
							@if( $categories->count() > 0 )
							<li class="dropdown {{ isActive($main_active, 'products') }}">
								<a href="{{ URL_DISPLAY_PRODUCTS }}">Products</a>
								<ul class="dropdown-menu multi-level" role="menu">
									@foreach( $categories->get() as $category)
									<li class="dropdown-submenu">
										@if( $category->parent_id == 0 )
										  
										<a href="{{ URL_DISPLAY_PRODUCTS . '/' . $category->slug }}"><span class="fa {{ $category->icon }}"></span>{{ $category->title }}</a>							
										<?php
							...
							@endif
						@elseif( $menu_item->page_id == 'pages')
							<?php
							$pages = App\Pages::where('status', '=', 'Active')->whereIn('id', (array) json_decode($menu_item->pages) );
							?>
							@if( $pages->count() > 0 )
								<li class="dropdown {{ isActive($main_active, 'pages') }}">
									<a href="javascript:void(0);">Pages</a>
									<ul class="dropdown-menu multi-level" role="menu">
										@foreach( $pages->get() as $page)
										<li class="dropdown-submenu">
											<a href="{{ URL_VIEW_PAGE . $page->slug }}"><span class="fa {{ $page->icon }}"></span>{{ $page->title }}</a>
										</li>
										@endforeach	
									</ul>
								</li>
							@endif
						@elseif( $menu_item->url == 'faq' )
						<li {{ isActive($main_active, 'faq') }}><a href="{{URL_FAQ_PAGE}}">FAQ</a></li>						
						@else
						<?php						
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<!--for home-->
				<li class="{{ isActive($main_active, 'home') }}"><a href="{{ PREFIX }}">Home</a></li>
				<!--for products-->
				<?php
				$categories = App\Category::where('status', '=', 'Active');
				?>
				...
				<li class="dropdown {{ isActive($main_active, 'products') }}">
					...
					<ul class="dropdown-menu multi-level" role="menu">
						...
						<li class="dropdown-submenu">
							@if( $category->parent_id == 0 )
							<a href="{{ URL_DISPLAY_PRODUCTS . '/' . $category->slug }}"><span class="fa fa-product-hunt"></span>{{ $category->title }}<span class="fa {{ $category->icon }}"></span></a>							
							<?php
							$subcats = App\Category::where('status', '=', 'Active')->where('parent_id', '=', $category->id);
							?>
							@if( $subcats->count() > 0 )
							<ul class="dropdown-menu">
								@foreach( $subcats->get() as $subcat)
									<li><a href="{{ URL_DISPLAY_PRODUCTS . '/' . $category->slug . '/' . $subcat->slug }}"> {{ getPhrase($subcat->title) }}</a> </li>
								@endforeach
							</ul>
							@endif
							@endif
				...
				<li class="{{ isActive($main_active, 'login') }}"><a href="{{ $url }}">{{ getPhrase('my_dashboard') }}</a></li>
              ...
              <span class="badge badge-success"></span><sup>{{$count = Auth::user()->newThreadsCount()}}</sup>
				...
				@else
					<li class="{{ isActive($main_active, 'login') }}"><a href="{{ URL_USERS_LOGIN }}">{{getPhrase('login')}}</a></li>
				@endif
